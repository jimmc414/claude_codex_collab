...
3. Keep commits scoped to one workflow's directory to maintain the separation between tooling
   ecosystems. Codex stages now include enforceable ready checklists—manage them with
   `python -m codex.pipeline_cli checklist <stage>` while progressing through the pipeline.
