...
function init() {
    ...
    read -p "Project name (or press Enter to skip): " proj_name
    
    if [ ! -z "$proj_name" ]; then
        mkdir -p "$proj_name"
        cd "$proj_name"
        echo "Created and moved to: /mnt/c/python/$proj_name"
...
function gh-upload-private() {
    ...
    echo "✅ Private repository created: https://github.com/$gh_user/$repo_name"
    echo "$gh_user/$repo_name" > .github_repo
}

# Update repo description
function gh-describe() {
    repo_name=$(basename "$PWD")
    
    if [ -z "$1" ]; then
        echo "Usage: gh-describe \"Your repository description\""
        return 1
    fi
...
function gh-info() {
    ...
    gh repo view "$repo_name" --json description,url,visibility,createdAt --jq '. | "Description: \(.description)\nURL: \(.url)\nVisibility: \(.visibility)\nCreated: \(.createdAt)"'
}
