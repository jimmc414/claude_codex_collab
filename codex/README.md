...
3. Operate ChatGPT Pro with those prompts. Update the ready checklist as items pass with
   `python -m codex.pipeline_cli checklist <stage> set ...`.
4. When a stage produces a document, store the approved output via
   `python -m codex.pipeline_cli capture <stage>` once the checklist shows only PASS.
5. Mark non-document stages complete with `python -m codex.pipeline_cli complete <stage>` after
   the checklist is satisfied.
6. `status` shows progress and any notes captured during the run.
...
- `python -m codex.pipeline_cli checklist <stage> show` – review or update ready checklist status
  (`set` to mark PASS/TODO, `reset` to revert to TODO).
- `python -m codex.pipeline_cli reset` – clear the current pipeline state (does not delete
  artifacts).

### Ready checklist gating

Stages that define a ready checklist stay in the `in_progress` state until each item is marked as
`PASS`. Use `python -m codex.pipeline_cli checklist <stage> show` to review outstanding work and the
`set` subcommand to note progress. Attempting to mark a stage complete before all items pass will
produce an error so the pipeline enforces the readiness threshold described in each prompt template.
