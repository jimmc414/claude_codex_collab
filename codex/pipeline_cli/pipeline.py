...
def _ready_marker(status: str) -> str:
    return "PASS" if status == "pass" else "TODO"


def _print_stage_checklist(stage, state: PipelineState) -> None:
    if not stage.ready_checklist:
        return
    statuses = state.get_ready_status(stage.key)
    print(indent("Ready checklist:", "    "))
    for index, item in enumerate(stage.ready_checklist, start=1):
        marker = _ready_marker(statuses.get(item, "todo"))
        line = f"{index}. [{marker}] {item}"
        print(indent(line, "        "))
...
def command_status(manager: StateManager) -> None:
    ...
    for stage_key, status in state.list_statuses():
        ...
        if stage.ready_checklist:
            _print_stage_checklist(stage, state)
...
def command_capture(args: argparse.Namespace, manager: StateManager) -> None:
    ...
    ready_complete = state.is_ready_complete(stage.key)
    if ready_complete:
        ...
        state.set_status(stage.key, "in_progress")
        state.stage_notes[stage.key] = (
            f"Artifact saved to {display_path}; ready checklist items remain TODO"
        )
        manager.save(state)
        print(f"Saved artifact to {display_path}. Stage remains in progress until checklist passes.")
        print(
            "Use 'python -m codex.pipeline_cli checklist {stage} show' to review outstanding items.".format(
                stage=stage.key
            )
        )
...
def command_complete(args: argparse.Namespace, manager: StateManager) -> None:
    ...
    try:
        ...
        print(str(exc))
        print(
            "Use 'python -m codex.pipeline_cli checklist {stage} show' to update the ready checklist.".format(
                stage=stage.key
            )
        )
        sys.exit(1)
...
def _resolve_checklist_item(stage, *, index: Optional[int], name: Optional[str]) -> str:
    if not stage.ready_checklist:
        raise RuntimeError(f"Stage '{stage.title}' does not define a ready checklist.")
    items = stage.ready_checklist
    if index is not None:
        if index < 1 or index > len(items):
            raise RuntimeError(
                f"Checklist index {index} is out of range for stage '{stage.key}'."
            )
    ...
    if name:
        for item in items:
            if item.lower() == name.lower():
                return item
        raise RuntimeError(
            f"Checklist item '{name}' not found. Use the 'show' subcommand to list available items."
        )
    raise RuntimeError("Either --index or --item must be provided to identify the checklist entry.")
...
def command_checklist(args: argparse.Namespace, manager: StateManager) -> None:
    ...
    if args.checklist_command == "reset":
        state.reset_ready(stage.key)
        if state.get_status(stage.key) == "complete":
            state.set_status(stage.key, "in_progress")
        manager.save(state)
        print(f"Reset ready checklist for {stage.title}. All items set to TODO.")
        return

    if args.checklist_command == "set":
        try:
            item = _resolve_checklist_item(stage, index=args.index, name=args.item)
        except RuntimeError as exc:
            print(str(exc))
            sys.exit(1)
        state.update_ready_item(stage.key, item, args.status)
        manager.save(state)
        print(
            "Marked '{item}' as {status} for {title}.".format(
                item=item,
                status=_ready_marker(args.status),
                title=stage.title,
            )
        )
        if state.is_ready_complete(stage.key) and state.get_status(stage.key) != "complete":
            print(
                "Ready checklist complete. Run 'python -m codex.pipeline_cli complete {stage}' to finalize the stage.".format(
                    stage=stage.key
                )
            )
        return

    print("Unknown checklist command. Use 'show', 'set', or 'reset'.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    checklist_parser = subparsers.add_parser(
        "checklist", help="Inspect or update a stage's ready checklist"
    )
    checklist_parser.add_argument("stage", choices=STAGE_ORDER, help="Stage key to inspect")
    checklist_sub = checklist_parser.add_subparsers(dest="checklist_command")
    checklist_sub.required = True

    checklist_sub.add_parser("show", help="Display ready checklist status")

    checklist_set = checklist_sub.add_parser(
        "set", help="Mark a ready checklist item as PASS or TODO"
    )
    target_group = checklist_set.add_mutually_exclusive_group(required=True)
    target_group.add_argument("--index", type=int, help="1-based index of the checklist item")
    target_group.add_argument("--item", help="Exact text of the checklist item")
    checklist_set.add_argument(
        "--status",
        choices=["pass", "todo"],
        required=True,
        help="Ready status to apply",
    )

    checklist_sub.add_parser("reset", help="Reset all ready checklist items to TODO")
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if args.command == "init":
        ...
        command_checklist(args, manager)
